...
def process_video_stream():
    ...
    cv2.startWindowThread()  # Qt 타이머 충돌 방지용

    executor = ThreadPoolExecutor(max_workers=4)
    depth_frame_counter = 0
    DEPTH_INFER_INTERVAL = 3

    vlm_frame_counter = 0
    VLM_INFER_INTERVAL = 300  # 약 5초 (30fps 기준)
    ...
    try:
        while not shared_state.stop_event.is_set():
            ...
            if depth_frame_counter >= DEPTH_INFER_INTERVAL:
            ...
            if vlm_frame_counter >= VLM_INFER_INTERVAL:
                Thread(target=run_vlm, args=(frame_ori.copy(),)).start()
                vlm_frame_counter = 0
